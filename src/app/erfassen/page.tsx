...
function ErfassenForm() {
  ...
  async function handleSave() {
    ...
    try {
      if (isEdit && editId) await updateDog(editId, form as DogUpdate)
      else await createDog(form as DogInsert)
      setDone(true)
    } catch (e: unknown) {
    ...
    <div className="flex items-center justify-center min-h-screen bg-slate-50">
      <div className="text-center max-w-sm px-6">
        ...
        <div className="flex gap-3 justify-center">
          <button onClick={() => router.push('/hunde')}
          ...
          {!isEdit && (
            <button onClick={() => { setForm({}); setDone(false); setSection(0) }}
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-6 px-4 md:py-8 md:px-6">
          ...
          {section === 1 && (
            <div>
              ...
              <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 text-xs text-indigo-600 mt-2">
                Grosseltern werden automatisch berechnet sobald die Elterntiere in der Datenbank sind.
              </div>
            </div>
          )}
          ...
          {section === 5 && (
            <div>
              <div className="grid grid-cols-2 gap-x-3">
                ...
                <Field label="Working-dog URL" value={form.workingdog_url ?? ''} onChange={v => set('workingdog_url', v)} placeholder="https://..." />
              </div>
              <div className="mb-4">
...
export default function ErfassenPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center h-96 text-slate-400 text-sm">Wird geladen...</div>}>
      <ErfassenForm />
    </Suspense>
  )
}