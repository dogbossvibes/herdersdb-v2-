...
function ErfassenPage() {
  ...
  if (done) return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '80vh', fontFamily: 'inherit' }}>
      <div style={{ textAlign: 'center', maxWidth: 400 }}>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
          <button onClick={() => router.push('/hunde')} style={{ background: 'rgba(99,102,241,0.2)', border: '1px solid rgba(99,102,241,0.4)', color: '#a5b4fc', borderRadius: 10, padding: '10px 20px', cursor: 'pointer', fontSize: 14, fontWeight: 600 }}>
            Zur Hundeliste
          </button>
          {!isEdit && (
            <button onClick={() => { setForm({}); setDone(false); setSection(0) }} style={{ background: '#6366f1', border: 'none', color: '#fff', borderRadius: 10, padding: '10px 20px', cursor: 'pointer', fontSize: 14, fontWeight: 600 }}>
              Weiteren Hund
            </button>
          )}
        </div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 13, color: '#64748b' }}>
            {Object.values(form).filter(v => v != null && v !== '').length} Felder ausgefuellt
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: 24 }}>
          ...
          <div>
            {SECTIONS.map((sec, i) => (
              <button key={i} onClick={() => setSection(i)} style={{
                width: '100%', background: section === i ? 'rgba(99,102,241,0.2)' : 'rgba(255,255,255,0.02)',
                border: section === i ? '1px solid rgba(99,102,241,0.4)' : '1px solid rgba(255,255,255,0.07)',
                color: section === i ? '#a5b4fc' : '#64748b',
                borderRadius: 10, padding: '10px 14px', cursor: 'pointer', fontSize: 13,
                fontWeight: 600, textAlign: 'left', marginBottom: 6, fontFamily: 'inherit',
              }}>
                {sec}
              </button>
            ))}
            <button onClick={handleSave} disabled={saving} style={{
              ...
              borderRadius: 10, padding: 12, cursor: saving ? 'not-allowed' : 'pointer',
              fontSize: 14, fontWeight: 700, fontFamily: 'inherit', marginTop: 16,
          ...
          <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 16, padding: '24px 28px' }}>
            ...
            {section === 1 && (
              <FormSection title="Stammbaum - Eltern">
            ...
            {section === 4 && (
              <FormSection title="Titel und Pruefungen">
            ...
            {section === 5 && (
              <FormSection title="Besitzer und Zuchter">
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 20px' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 24, paddingTop: 20, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
              <button onClick={() => setSection(Math.max(0, section - 1))} disabled={section === 0} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: section === 0 ? '#334155' : '#94a3b8', borderRadius: 10, padding: '9px 20px', cursor: section === 0 ? 'not-allowed' : 'pointer', fontSize: 13, fontFamily: 'inherit' }}>
                Zurueck
              </button>
              {section < SECTIONS.length - 1 ? (
                <button onClick={() => setSection(section + 1)} style={{ background: 'rgba(99,102,241,0.2)', border: '1px solid rgba(99,102,241,0.4)', color: '#a5b4fc', borderRadius: 10, padding: '9px 20px', cursor: 'pointer', fontSize: 13, fontWeight: 600, fontFamily: 'inherit' }}>
                  Weiter
                </button>
              ) : (
                <button onClick={handleSave} disabled={saving} style={{ background: '#6366f1', border: 'none', color: '#fff', borderRadius: 10, padding: '9px 24px', cursor: 'pointer', fontSize: 13, fontWeight: 700, fontFamily: 'inherit' }}>
                  {saving ? 'Speichern...' : 'Speichern'}
                </button>
              )}
            </div>
...
function FormSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <div style={{ fontSize: 16, fontWeight: 700, color: '#f8fafc', marginBottom: 20 }}>{title}</div>
      {children}
    </div>
  )
}
...
function Field({ label, value, onChange, type = 'text', placeholder }: {
  ...
  const inp2 = { width: '100%', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#f1f5f9', borderRadius: 10, padding: '10px 14px', fontSize: 13, boxSizing: 'border-box' as const, fontFamily: 'inherit' }
  return (
    <div style={{ marginBottom: 16 }}>
      <label style={{ fontSize: 12, color: '#94a3b8', display: 'block', marginBottom: 6, fontWeight: 600 }}>{label}</label>
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} style={inp2} />
    </div>
  )
}
...
function SelectField({ label, value, onChange, options }: {
  ...
  const sel2 = { width: '100%', background: '#1e293b', border: '1px solid rgba(255,255,255,0.1)', color: '#f1f5f9', borderRadius: 10, padding: '10px 14px', fontSize: 13, boxSizing: 'border-box' as const, fontFamily: 'inherit' }
  return (
    <div style={{ marginBottom: 16 }}>
      <label style={{ fontSize: 12, color: '#94a3b8', display: 'block', marginBottom: 6, fontWeight: 600 }}>{label}</label>
      <select value={value} onChange={e => onChange(e.target.value)} style={sel2}>
